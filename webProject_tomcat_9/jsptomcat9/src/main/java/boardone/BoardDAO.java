...
public class BoardDAO {
	...
	public void insertArticle(BoardVO article) {
		...
		try {
			...
			sql = "insert into board(num,writer,email,subject,pass,regdate,ref,step,depth,content,ip,filename,origfilename) values(board_seq.nextval,?,?,?,?,?,?,?,?,?,?,?,?)";
			pstmt = conn.prepareStatement(sql);
			...
			pstmt.setString(12, article.getOrigfilename());
			pstmt.executeUpdate();
	...
	public BoardVO getArticle(int num) {
		...
		try {
			...
			if (rs.next()) {
				...
				article.setOrigfilename(rs.getString("origfilename"));
			}
	...
	public BoardVO updateGetArticle(int num) {
		...
		try {
			...
			pstmt = conn.prepareStatement("select * from board whrer num = ?");
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				article = new BoardVO();
	...
	public int updateArticle(BoardVO article) {
		...
		System.out.println(article.getPass());
		System.out.println(article.getFilename());
		System.out.println(article.getOrigfilename());
		...
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
				dbpasswd = rs.getString("pass");// 비밀번호비교
				if (dbpasswd.equals(article.getPass())) {
					sql = "update board set writer=?,email=?,subject=?,content=?,filename=?,origfilename=? where num=?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setString(1, article.getWriter());
					pstmt.setString(2, article.getEmail());
					pstmt.setString(3, article.getSubject());
					...
					pstmt.setString(6, article.getOrigfilename());
